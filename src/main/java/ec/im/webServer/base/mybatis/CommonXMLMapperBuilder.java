...
public class CommonXMLMapperBuilder {
	...
	public static void main(String[] args) throws Exception {
		FileSystemResource resource = new FileSystemResource("D:\\code\\2.4.5\\ec-user\\ec-user-provider\\src\\main\\resources\\com\\kyexpress\\ec\\user\\provider\\mapper\\UserInfoMapper.xml");
	...
	private Resource parse(Resource resource) throws Exception {
	    ...
	    byte[] copy = new byte[resource.getInputStream().available()];
	    resource.getInputStream().read(copy);

		XPath xpath = XPathFactory.newInstance().newXPath();

		DocumentBuilder builder = DocumentBuilderFactory.newInstance().newDocumentBuilder();
		builder.setEntityResolver(new XMLMapperEntityResolver());
		Document document = builder.parse(new ByteArrayInputStream(copy));
		Node rootNode = (Node) xpath.evaluate("/mapper", document, XPathConstants.NODE) ;
		if (rootNode == null) {
			logger.info("Invalid XML Map file, ignore xml file {}", absolutePath);
			return null;
		}
		...
		if (tableNameNode == null) {
			logger.info("Can't found {} sql segment, ignore map {}", baseTableName, absolutePath);
			return null;
		}
		...
		if (resultMapNode == null) {
			logger.info("Can't found {} resultMap, ignore map {}", baseTableName, absolutePath);
			return null;
		}
		//组装表的映射类
		...
		if (idNode == null) {
		    // 不处理无主键的mapper
            return new ByteArrayResource(copy, fileName);
        }
		...
		return rewriteXmlMapper(fileName, copy, table, existMethodSet);
	}
	 ...
	 */
	private Resource rewriteXmlMapper(String fileName, byte[] data, CommonTable table, HashSet<String> existMethodSet)
...
//		String xmlMapperStr = new String(Files.readAllBytes(resource.getFile().toPath()), "utf-8");
		...
		return new ByteArrayResource(xmlContext.getBytes("utf-8"), fileName);
	}
